...
void FluidSystem::Init(ID3D12Device* device, DXGI_FORMAT rtvFormat,
    ...
    // 1) ComputePipelineState を既存クラスで初期化
    {
        // ルートシグネチャは ComputePipelineState 内部で生成可能
        m_computePS.SetDevice(device);
        m_computePS.SetShader(L"ParticleCS.cso");
        m_computePS.Create();
...
void FluidSystem::Simulate(ID3D12GraphicsCommandList* cmd, float dt) {
    if (m_useGpu) {
        // GPU シミュレーション
        cmd->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::UAV(m_particleBuffer.Get()));
